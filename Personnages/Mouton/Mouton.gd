...
var _mvtF_leader : Sprite = null
var _mvtF_remaining : float = 0.0

func _enter_tree():
    getLevelManager().sheep_follow_leader_start.connect(_on_follow_leader_start)
    getLevelManager().sheep_follow_leader_stop.connect(_on_follow_leader_stop)
    getLevelManager().sheepArray.push_back(self)

func _exit_tree():
    getLevelManager().sheep_follow_leader_start.disconnect(_on_follow_leader_start)
    getLevelManager().sheep_follow_leader_stop.disconnect(_on_follow_leader_stop)
